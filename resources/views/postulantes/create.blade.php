<?php $titulo = 'Nuevo Postulante - SIGIE'; ?>

<?php
$periodoDefault = old('periodo_id', $periodoActivo['id'] ?? '');
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        <h2 class="mb-1">Nuevo postulante</h2>
        <p class="text-muted mb-0">Registrar datos personales y académicos del postulante nacional</p>
    </div>
...
<form action="<?= e(url('/postulantes/store')) ?>" method="POST">
    <div class="card mb-4">
                ...
                <div class="col-md-6">
                    ...
                    <select name="periodo_id" class="form-select" required>
                        ...
                        <?php foreach ($periodos as $periodo): ?>
                            <option value="<?= e($periodo['id']) ?>" <?= selected_value($periodoDefault, $periodo['id']) ?>>
                                <?= e($periodo['codigo']) ?> - <?= e($periodo['gestion']) ?>/<?= e($periodo['semestre']) ?>
                                (<?= e($periodo['estado']) ?>)
                            </option>
                        <?php endforeach; ?>
                    </select>
                    <small class="text-muted">Normalmente debe quedar en la gestión activa.</small>
                </div>
            </div>
        </div>
    </div>
        ...
        <div class="card-body">
            <div class="row g-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="colegio" class="form-control" value="<?= e(old('colegio')) ?>" required>
                </div>

                <div class="col-md-4">
                    <label class="form-label required">Ciudad</label>
                    <input type="text" name="ciudad" class="form-control" value="<?= e(old('ciudad', 'Santa Cruz')) ?>" required>
                </div>

                <div class="col-md-4">
                    <label class="form-label">Título de bachiller</label>
                    <input type="text" name="titulo_bachiller" class="form-control" value="<?= e(old('titulo_bachiller', 'Bachiller en Humanidades')) ?>">
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <select name="carrera_principal_id" class="form-select" required>
                        ...
                        <?php foreach ($carreras as $carrera): ?>
                            <option value="<?= e($carrera['id']) ?>" <?= selected_value(old('carrera_principal_id'), $carrera['id']) ?>>
                ...
                <div class="col-md-6">
                    ...
                    <select name="carrera_secundaria_id" class="form-select" required>
                        <option value="">Seleccione una carrera</option>
                        <?php foreach ($carreras as $carrera): ?>
                            <option value="<?= e($carrera['id']) ?>" <?= selected_value(old('carrera_secundaria_id'), $carrera['id']) ?>>
                                <?= e($carrera['codigo']) ?> - <?= e($carrera['nombre']) ?>
                            </option>
                        <?php endforeach; ?>
                    </select>
